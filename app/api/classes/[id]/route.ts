...
export async function PUT(request: Request, { params }: Props) {
  try {
    ...
    const { nama_kelas, deskripsi } = await request.json();

    const [result] = await pool.query(
      'UPDATE classes SET nama_kelas=?, deskripsi=? WHERE id=?',
      [nama_kelas, deskripsi, classId]
    );
    ...
    return NextResponse.json({ success: true, data: { id: classId, nama_kelas, deskripsi } });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}
